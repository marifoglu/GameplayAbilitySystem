...
void UAuraAbilitySystemComponent::ForEachAbility(const FForEachAbility& Delegate)
{
	...
	for (const FGameplayAbilitySpec& AbilitySpec : GetActivatableAbilities())
	{
		...
		{
			UE_LOG(LogDante, Error, TEXT("Failed to execute delegate in %hs"), __FUNCTION__);
		}
	}
}
...
void UAuraAbilitySystemComponent::UpdateAbilityStatuses(int32 Level)
{
	...
	{
		if (!Info.AbilityTag.IsValid()) continue; // check Ability Tag here first
		if (Level < Info.LevelRequirement) continue; // check level requirement

		if (GetSpecFromAbilityTag(Info.AbilityTag) == nullptr) // Assign now!
		{
	...
	const FGameplayTag& Slot)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		const FGameplayTag& Status = GetInputTagFromSpec(*AbilitySpec);

		const bool bStatusValid = Status == GameplayTags.Abilities_Status_Equipped || Status == GameplayTags.Abilities_Status_Unlocked;
		if (bStatusValid)
		{
			// Remove this InputTag (slot) from any other ability that has it. (Unequip whatever is currently in the slot).
